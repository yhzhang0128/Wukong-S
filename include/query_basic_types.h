/**
 * @file query_basic_types.h
 * @brief Data structures for SPARQL and C-SPARQL queries
 */
...
struct request_or_reply{
    ...
    int stream_row_num(){
        int ret = silent_row_num;
        int sz = stream_info.atomic_result_table.size();
        for(int i = 0; i < sz; i++){
            ret *= stream_info.atomic_result_table[i];
        }
        return ret;
    }
