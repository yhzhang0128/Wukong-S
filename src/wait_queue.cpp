/**
 * @file wait_queue.cpp
 * @brief Methods in wait_queue class
 */

#include "wait_queue.h"

bool wait_queue::is_ready(int parent_id){
    return internal_item_map[parent_id].count==0;
}
request_or_reply wait_queue::get_merged_reply(int parent_id){
...
void wait_queue::put_parent_request(request_or_reply& req,int count){
    item data;
    data.count=count;
...
void wait_queue::put_reply(request_or_reply& reply){
