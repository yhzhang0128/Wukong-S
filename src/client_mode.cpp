/**
 * @file client_mode.cpp
 * @brief Routines for iterative shell
 */
...
void iterative_shell(client* clnt){
	...
	while(true){
		...
		if(cfg->m_id==0 && cfg->t_id==0){
			...
			for(int i=0;i<cfg->m_num;i++){
				for(int j=0;j<cfg->client_num;j++){
					...
					cfg->node->Send(cfg->m_id, i, j, input_str);
				}
			}
		} else {
			input_str=cfg->node->Recv(0);
		}
		...
		} else if(input_str=="latency"){
			//if (cfg->m_id == 0)
            stream_result_recorder->print_latency(cfg->m_id);
        } else if (input_str == "throughput"){
            stream_result_recorder->print_throughput(cfg->m_id);
		} else if(input_str=="world"){
			cout << "This is world " << cfg->m_id;
		} else if(input_str=="metadata"){
			metadata_manager->print_memory_usage();
        } else if(input_str=="silent"){
            global_stream_query_silent = true;
            global_dispatch_debugging = false;
            global_server_debugging = false;
		} else {
			//handle queries here
